...
class WhisperXBackend:
    ...
    def transcribe_raw(
        ...
        `on_progress` (optional) is a callable invoked by WhisperX after every
        internal batch with the percent-complete (0..100). Plumbed through to
        `model.transcribe(progress_callback=...)`.
        """
    ...
    def align_words(
        ...
        `on_progress` (optional) is a percent-complete callback for the alignment loop.
        """
        import whisperx  # type: ignore
    ...
    def assign_speakers(self, audio_path: Path, aligned: dict[str, Any], opts: TranscriptionOptions) -> dict[str, Any]:
        ...
        # HF_TOKEN is only required to *download* the gated pyannote model.
        # If the model is already in the local HF cache (or HF_HUB_OFFLINE=1
        # is set), the token is unnecessary — pyannote loads from disk and
        # never hits the hub.
        import os
        offline = os.environ.get("HF_HUB_OFFLINE") in ("1", "true", "True")
        if not opts.hf_token and not offline:
            raise RuntimeError(
                ...
                "If you've already downloaded the model on this machine, "
